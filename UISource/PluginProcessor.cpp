...
const float kMaximumDelay = 0.02;
const float kMaximumSweepWidth = 0.02;
...
{
    const juce::StringArray waveformChoices = juce::StringArray("Triangular", "Sine", "Square");
    const juce::StringArray interpolationChoices = juce::StringArray("Linear", "Quadratic", "Cubic");
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
    params.push_back(std::make_unique<juce::AudioParameterFloat>("DELAY", "Delay", 0.0f, 1.0f, 0.5f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>("SWEEP", "Sweep Width", 0.0f, 1.0f, 0.12f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("FREQUENCY", "Frequency", 0.0f, 10.0f, 0.5f));
    
    params.push_back(std::make_unique<juce::AudioParameterChoice>("WAVEFORM", "waveform", juce::StringArray { "Sine", "Triangle", "Square", "Saw"}, 0));
    params.push_back(std::make_unique<juce::AudioParameterChoice>("INTERPOLATION", "interpolation", juce::StringArray{"Linear", "Square", "Cubic"}, 0));

    params.push_back(std::make_unique<juce::AudioParameterFloat>("WETDRY", "Wet/Dry", 0.0f, 1.0f, 0.5f));
    params.push_back(std::make_unique<juce::AudioParameterBool>("STEREO", "stereo", true, "stereo"));
...
void FlangerAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (channel = 0; channel < numInputChannels; ++channel)
    {
        ...
        float speedP = params.getRawParameterValue("SPEED")->load();
        float delayP = params.getRawParameterValue("DELAY")->load();
        float wetP = params.getRawParameterValue("WETDRY")->load();
        float fbP = params.getRawParameterValue("FEEDBACK")->load();
        float sweepP = params.getRawParameterValue("SWEEP")->load();
        float gP = params.getRawParameterValue("G")->load();
        float timeP = params.getRawParameterValue("TIME")->load();
        int polarityP = params.getRawParameterValue("POLARITY")->load();
        int interpolP = params.getRawParameterValue("INTERPOLATION")->load();
        int waveP = params.getRawParameterValue("WAVEFORM")->load();
        int stereoP = params.getRawParameterValue("STEREO")->load();
